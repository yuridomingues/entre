  ...
  return <div className="scale-doodle">
    <section className="scale-doodle-card">
      <header className="scale-doodle-head">
        ...
        <div className="scale-doodle-ratio"><b>{fmt(ratio)}×</b><span>{current.measure}</span></div>
      </header>
      ...
      >
        ...
        <div className="doodle-object previous" key={`p-${index}`} style={{height:`${Math.max(.25,previousHeight)}%`}}>
        ...
        <div className="doodle-object current" key={`c-${index}`} style={{height:`${currentHeight}%`}}>
          ...
          <div className="doodle-label"><strong>{current.name}</strong><span>{current.size}</span></div>
        </div>

        <div className="doodle-measure">
      ...
      <p className="scale-doodle-sentence">{previous.name} mede <strong>{previousPct<1?"menos de 1":fmt(previousPct)}%</strong> de {current.name} nesta comparação.</p>
      ...
      <p className="scale-doodle-hint">use as setas ou deslize no celular</p>
    </section>
